    ...
    <a-row :gutter="10" style="background-color: #ececec; padding: 10px; margin: -10px">
      <a-card :bordered="false">
        ...
        <a-input-search
          ...
          placeholder="请输入明细类名称"
          v-model="queryParam.kindName"
          @search="onSearch"
        ></a-input-search>
        <a-button @click="searchReset(1)" style="margin-left: 20px" icon="redo">重置</a-button>
        <a-text style="color: blue; paddingLeft: 16px">注意：如果找不到，请至菜单【基础资料->明细类】新增！</a-text>
        <!--明细类列表-->
        <a-table
          ref="table"
...
<script>
  ...
  import { getKindNoSmallList } from '@/api/api'

  export default {
    name: 'JSelectKindNoSmallModal',
    components: {},
    props: ['modalWidth'],
    data() {
      return {
    ...
    methods: {
      loadData(arg) {
        ...
        getKindNoSmallList(params).then((res) => {
          if (res.success) {
