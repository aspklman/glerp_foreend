  ...
  export default {
    ...
    data() {
      return {
        ...
        columns: [
          {
            title: this.$t('sec.secNo'),
            align: 'center',
            dataIndex: 'secNo'
          },
          {
            title: this.$t('sec.secName'),
            align: 'center',
            dataIndex: 'secNm'
          },
          {
        ...
        selectedKeys: [],
        secNmArr: [],
        title: this.$t('common.select') + this.$t('sec.sec'),
        ipagination: {
          current: 1,
          pageSize: 10,
    ...
    methods: {
      ...
      handleSubmit() {
        ...
        for (let i = 0, len = this.selectedRowKeys.length; i < len; i++) {
          this.getSecNos(this.selectedRowKeys[i])
        }
        that.$emit('ok', that.secNmArr.join(','))
        that.close()
      },
      // 遍历匹配,获取【组别编号】
      getSecNos(rowId) {
        ...
        for (let i = 0, len = dataSource.length; i < len; i++) {
          if (rowId === dataSource[i].id) {
            this.secNmArr.push(dataSource[i].secNo)
          }
        }
      },
...
<style scoped>
  ...
  #components-layout-demo-sec-trigger .trigger {
    font-size: 18px;
    line-height: 64px;
    padding: 0 24px;
    cursor: pointer;
